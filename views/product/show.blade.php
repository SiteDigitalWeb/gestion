...
<div class="block">
    ...
    'id' => 'defaultForm'
     ...
     <label class="col-md-3 control-label">Términos y Condiciones</label>
      <div class="col-md-12">
        {{ Form::textarea('descripcion', '', ['id' => 'editor2','placeholder'=>'Ingrese contenido']) }}
      </div>
    </div>

                                        </div>

                                        {{Form::hidden('identificador',  Request::segment(3), array('class' => 'form-control','placeholder'=>'Ingrese descripción'))}}
 ...
 <div class="block full">
    ...
    @foreach($productos as $producto)
        <tr>
            ...
            <td class="text-center">{{ $producto->posti }}</td>
            <td>{{ $producto->iva }}</td>
            <td class="text-center">
                <a href="{{ route('ge.product.edit', $producto->id) }}" class="btn btn-primary" title="Editar producto">
                    <i class="fa fa-pencil-square-o sidebar-nav-icon"></i>
                </a>
                <form action="{{ route('ge.product.destroy', $producto->id) }}" 
      method="POST" 
